...
const createMenu = (pedido) => {
  const novoObjeto = {};
  Object.assign(novoObjeto, { 
    fetchMenu: () => pedido,
    consumption: [],
    order: (recebe) => novoObjeto.consumption.push(recebe),
    pay: () => {
      let sum = 0;
      const food = Object.entries(pedido.food);
      const drink = Object.entries(pedido.drink);
      const complete = food.concat(drink);
      const consumido = novoObjeto.consumption;

      consumido.forEach((e) => {
        complete.forEach((e2) => {
          if (e === e2[0]) sum += (e2[1] * 1.1);
        });
      });
      return sum;
    },
